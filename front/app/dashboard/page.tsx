...
const Dashboard = () => {
  ...
  const [isConfusionMatricesOpen, setIsConfusionMatricesOpen] =
    useState(false);
  const [isDataDistributionOpen, setIsDataDistributionOpen] = useState(false);
  const [isAnalyticsModalOpen, setIsAnalyticsModalOpen] = useState(false);
  const [isNormalisationStatsOpen, setIsNormalisationStatsOpen] =
    useState(false);
      ...
      {flashMessages.length > 0 && (
        <div className="fixed top-4 right-4 z-50 space-y-3 max-w-md w-full">
          {flashMessages.map((message) => (
            <div
              key={message.id}
              className={`p-4 rounded-2xl border-2 shadow-lg animate-fade-in-up ${getFlashMessageStyles(
                message.type
              )}`}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-3">
                  <span className="text-xl mt-0.5">
                    {getFlashMessageIcon(message.type)}
                  </span>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-slate-500">
                  Bienvenue sur votre espace de surveillance
                </p>
              </div>
            </div>
              ...
              {[
                {
                  ...
                  desc: "Générer un rapport détaillé",
                },
                {
                  icon: "🤖",
                  title: "Entraîner Modèle",
                  desc: "Optimiser les performances",
                },
                {
                  icon: "📋",
                  title: "Matrices Confusion",
                  desc: "Analyser les performances",
                },
                {
                  icon: "⚙️",
                  title: "Paramètres",
                  desc: "Configurer le système",
                },
                {
                  icon: "📊",
                  title: "Distribution Données",
                  desc: "Analyser le déséquilibre des classes",
                },
              ].map((action, index) => (
                <button
                  key={index}
      ...
      <TransactionnelModal
        ...
        onSave={(transactionResult) => {
          if (transactionResult) {
            ...
            let messageText =
              "La transaction a été analysée et approuvée avec succès.";

            if (transactionResult.type === "fraude") {
              messageType = "error";
              messageTitle = "Fraude Détectée !";
              messageText =
                "Transaction bloquée - Activité suspecte détectée.";
            }
